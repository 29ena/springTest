...
public class UserControllerTest extends ControllerTestEnv{
	...
	* Method 설명 : 사용자 전체 리스트 테스트
	...
	public void userListTest() throws Exception {
		...
		assertEquals(108, ((List<UserVo>)mav.getModelMap().get("userList")).size());
	}
	...
	* Method 설명 : 사용자 페이징 리스트 테스트
	...
	/**
	 * 
	* Method : userPagingListWithoutParamerTest
	* 작성자 : PC20
	* 변경이력 :
	* @throws Exception
...
//		assertEquals(10, pageVo.getPageSize());
	...
	* Method 설명 : 사용자 입력 화면 요청
	...
	/**
	 * 
	* Method : userFormPostSuccessPostTest
	* 작성자 : PC20
	* 변경이력 :
	...
	* Method 설명 : 사용자 등록 테스트 (Fail 시나리오)
	...
	public void userFormPostFailTest() throws Exception {
		...
		mockMvc.perform(fileUpload("/user/form")
				.file(file)
				.param("userId", "user100")	// 이미 존재하는 아이디
				.param("name", "대덕인")
				.param("alias","중앙로")
				.param("pass", "1234")
				.param("addr1", "대전광역시 중구 중앙로 76")
