...
public class BatchDaoTest extends LogicTestEnv{
	...
	* Method 설명 : 배치 실행 데이터 생성 테스트
	...
	public void insertBatchTest() {
		...
		batchVo.setSt("01");	// 진행 실행상태 : 01 - 진행중
		
		/***When***/
		logger.debug("before insertBatch batchVo.getBid() : {}", batchVo.getBcd());
		int insertCnt = batchDao.insertBatch(batchVo);
		logger.debug("after insertBatch batchVo.getBid() : {}", batchVo.getBcd());
		/***Then***/
	...
	* Method 설명 : 배치데이터 업데이트 테스트
	...
	public void updateBatchTest() {
		...
		batchVo.setSt("02");	// 진행 실행상태 : 01 - 진행중
