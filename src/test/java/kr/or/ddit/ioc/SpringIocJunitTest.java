...
public class SpringIocJunitTest {
	...
	// field 기준 boardService, boardService2 : spring boardService (scope = singleton)
	// 			boardServiceConstructor : spring boardServiceConstructor bean (scope = singleton)
	// 1. boardeService, boardService2 : 같아야함
	...
	// boardDaoPrototype : spring boardDaopPrototype(scope = prototype)
	// boardDaoPrototype2 : spring boardDaopPrototype(scope = prototype)
	// 1. boardDao boardDaoPrototype : spring bean id가 다르므로 다른 객체
	// 2. boardDaoPrototype, boarDaoPrototype2 : 두 객체는 같은 스프링 빈이지만 scope가
	//											 prototype이므로 다른 객쳐에여 한다.
	...
	* Method 설명 : 스프링 컨테이너의 singleton scope 테스트
	...
	public void springPrototypeScopeTest() {
		/***Given***/
		// 1. boardDao boardDaoPrototype : spring bean id가 다르므로 다른 객체
		// 2. boardDaoPrototype, boardDaoPrototype2 : 두 객체는 같은 스프링 빈이지만 scope가
		//											  prototype이므로 다른 객체여아 한다.
